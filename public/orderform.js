...
function register(){
	...
	let pas = document.getElementById("reg_pas").value;
	let pri = document.getElementById("select_div").value;
	let pri_val;
	

	if (!(name.localeCompare("")==0) && !(pas.localeCompare("")==0)){
		...
		let new_user = {"name":name.toLowerCase(),"password":pas,"privacy":pri_val};
		...
		alert("Text boxes cant be empty");
	}
}

function login(){
	let name = document.getElementById("log_name").value;
	let pas = document.getElementById("log_pas").value;

	if (!(name.localeCompare("")==0) && !(pas.localeCompare("")==0)){
		let new_user = {"name":name.toLowerCase(),"password":pas};
...
function toggle_privacy(){
	let pri = document.getElementById("select_div_pro").value;
	let pri_val;
	...
	let req = new XMLHttpRequest();
        ...
        req.open("PUT", "/toggle");
		req.setRequestHeader("Content-Type", "application/json");
        req.send(JSON.stringify({privacy:pri_val}));
}